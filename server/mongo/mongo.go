...
func NewClient(connection string) (Client, error) {
	monitor := &event.CommandMonitor{
		Started: func(_ context.Context, evt *event.CommandStartedEvent) {
			fmt.Printf("[mongo] Started: %s %s %s\n", evt.CommandName, evt.DatabaseName, evt.Command)
		},
		Succeeded: func(_ context.Context, evt *event.CommandSucceededEvent) {
			fmt.Printf("[mongo] Succeeded: %s %s %s\n", evt.CommandName, evt.Duration, evt.Reply)
		},
		Failed: func(_ context.Context, evt *event.CommandFailedEvent) {
			fmt.Printf("[mongo] Failed: %s %s %s\n", evt.CommandName, evt.Duration, evt.Failure)
		},
	}
	time.Local = time.UTC
	c, err := mongo.NewClient(options.Client().ApplyURI(connection).SetMonitor(monitor))
...
func (mc *mongoCollection) ReplaceOne(ctx context.Context, filter interface{}, replacement interface{}, opts ...*options.ReplaceOptions) (*mongo.UpdateResult, error) {
	return mc.coll.ReplaceOne(ctx, filter, replacement, opts[:]...)
}
